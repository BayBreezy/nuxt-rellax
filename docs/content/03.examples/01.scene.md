...
description: A layered night-sky scene built with a single useRellax call and data-rellax-speed attributes on each layer.
navigation:
  icon: lucide:mountain-snow
---

::prose-callout
One `useRellax` instance targeting a CSS class manages every layer. Individual speeds are set with `data-rellax-speed` — no separate directive call per element needed.
::
...
#code
<!-- automd:file src="../../app/components/content/ExampleScene.vue" code -->

```vue [ExampleScene.vue]
...
::prose-callout{variant="tip" title="CSS selector + data-rellax-speed"}
When multiple elements share the same parallax wrapper but need individual speeds, using a CSS class selector with `data-rellax-speed` on each element is more efficient than creating one `useRellax` instance per element.
::
