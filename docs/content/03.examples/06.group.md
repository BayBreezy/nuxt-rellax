---
title: Group Control
description: Six elements at different speeds managed by a single useRellaxGroup call. Group-level refresh, destroy, and reinitialize apply to all instances at once.
navigation:
  icon: lucide:layers-3
---

`useRellaxGroup` accepts an array of `{ el, options }` objects and returns a single control object. Calling `destroy()` kills all six instances; `reinitialize()` brings them all back.
...
#code
<!-- automd:file src="../../app/components/content/ExampleGroup.vue" code -->

```vue [ExampleGroup.vue]
...
::prose-callout{variant="info" title="Access individual instances"}
The return value also includes `instances` — an array of each individual `useRellax` return value — so you can target specific elements if needed.

```ts
const { instances, refresh } = useRellaxGroup([...])

// Control just the first element
instances[0].destroy()

// Check whether a specific one is active
console.log(instances[2].instance.value) // RellaxInstance | null
```
::
