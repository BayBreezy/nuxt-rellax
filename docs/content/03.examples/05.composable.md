...
description: An interactive demo of the useRellax return value — call refresh, destroy, and reinitialize from buttons and watch the instance respond in real time.
navigation:
  icon: lucide:terminal
---

`useRellax` returns `{ instance, refresh, destroy, reinitialize }`. This example wires those directly to buttons so you can see exactly what each method does to the running Rellax instance.
...
<!-- automd:file src="../../app/components/content/ExampleComposable.vue" code -->

```vue [ExampleComposable.vue]
...
::prose-callout{variant="tip" title="Options getter for dynamic values"}
Passing `() => ({ speed: speed.value })` instead of `{ speed: speed.value }` means `reinitialize()` re-reads `speed.value` at call time. With a plain object, the speed is captured once at setup and never updated.
::
